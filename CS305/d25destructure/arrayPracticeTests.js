...
const findNumSum = myExports.findNumSum;
...
describe("filter practice", function () {
    ...
    it("includes even number", function () {
        assert.strictEqual(includesEvenNum(0), true);
        assert.strictEqual(includesEvenNum(15), false);
    });

    /* complete the following similar to includes even test */
    it("includes even age ", function () {
        assert.strictEqual(includesEvenAge(6), true);
        assert.strictEqual(includesEvenAge(15), false);
    });
...
describe("reduce practice", function () {
    ...
    it("find sum of numbers", function () {
        assert.deepEqual(findNumSum(numArray), 422);
        assert.deepEqual(numArray, [5, 0, 7, 77, -20, 300, 51, 2]);  //test for pure function
    });

    it("find average of numbers", function () {
        assert.deepEqual(findNumAvg(numArray), 52.75);

    });

    it("find max of numbers", function () {
        assert.deepEqual(findMaxNum(numArray), 300);
    });
